...
# Server function ####
metadata_widget_server <- function(
  id,
  ...
  moduleServer(
    ...
    function(
      ...
      login = reactive({
        login_widget_output$login()
      })
      baseurl = reactive({
        login_widget_output$baseurl()
      })
      username = reactive({
        login_widget_output$username()
      })
      password = reactive({
        login_widget_output$password()
      })
      ...
      metadata.files = reactive({
        directory_widget_output$metadata.files()
      })
        ...
        if (login() & loginFetch()) {
          ...
          if (file.exists(file) & !dir.exists(file)) {
            ...
            if (!"data.frame" %in% class(info)) {
              info = data.frame(
                info = "unavailable. Please re-download metadata."
              )
            }
      ...
      dataElementGroups = reactive({
        if (login() & loginFetch()) {
          ...
          url <- paste0(
            baseurl(),
            "api/dataElementGroups.json?fields=:all&paging=false"
          )
          ...
          deg = map_df(
            1:length(dataElementGroups$dataElementGroup),
            ~ merge(
              dataElementGroups[.x, 1:2],
              dataElementGroups$dataElements[[.x]],
              all = T
            )
          ) %>%
      ...
      categoryCombos = reactive({
        if (login() & loginFetch()) {
          ...
          url <- paste0(
            baseurl(),
            "api/categoryCombos.json?fields=",
            paste(cols, collapse = ","),
            "&paging=false"
          )
      ...
      categoryOptionCombos = reactive({
        if (login() & loginFetch()) {
          ...
          url <- paste0(
            baseurl(),
            "api/categoryOptionCombos.json?fields=",
            paste(cols, collapse = ","),
            "&paging=false"
          )
      ...
      dataElementDictionary = reactive({
        if (login() & loginFetch()) {
          ...
          if (nrow(ds) > 0 && 'dataSetElements.id' %in% names(ds)) {
            dsde = map_df(
              1:length(ds$dataSet),
              ~ map_df(ds$dataSetElements.id[[.x]], ~ as.matrix(.x))
            ) %>%
      ...
      indicator_formula_translator = function(num_denom, id_names) {
        ids_between_braces = str_extract_all(num_denom, "\\{.*?\\}")[[1]] %>%
          gsub("\\{|\\}", "", .)
        ...
        if (login() & loginFetch()) {
          ...
          translated =
            ...
            mutate(
              numerator = map_chr(
                numerator.ids,
                ~ indicator_formula_translator(.x, id_names)
              ),
      ...
      orgUnitLevels = reactive({
        ...
        if (login() & loginFetch()) {
          ...
          url <- paste0(
            baseurl(),
            "api/organisationUnitLevels.json?fields=",
            paste(cols, collapse = ","),
            "&paging=false"
          )
      ...
      orgUnitDuplicates = reactive({
        ...
        orgUnitDuplicates = inner_join(orgUnits(), duplicates, by = 'name')
      ...
      geoFeatures_download = function(level = 2, .pb = NULL) {
        ...
        url <- paste0(
          baseurl(),
          "api/organisationUnits.geojson?level=",
          level,
      ...
      geoFeatures = reactive({
        ...
        if (login() & loginFetch()) {
          ...
          geosf.$longitude[is_point] <- st_coordinates(geosf.[is_point, ])[,
            "X"
          ]

          # add level.name
          geosf.$levelName = orgUnitLevels$levelName[match(
            geosf.$level,
            orgUnitLevels$level
          )]
          ...
          # add parent name
          orgUnits = orgUnits()
          geosf.$parentName = orgUnits$parent[match(
            geosf.$parent,
            orgUnits$parent.id
          )]
        } else {
