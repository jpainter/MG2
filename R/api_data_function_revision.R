...
#' @param YrsPrevious Number of years of data
#' @param orgUnits Level of data (default = LEVEL-1)
...
api_data = function(
  ...
  if (periodType == 'Monthly') {
    periods = date_code(YrsPrevious = YrsPrevious) %>% unique
  }
  if (periodType == 'Weekly') {
    periods = date_code_weekly(YrsPrevious = YrsPrevious) %>% unique
  }
        ...
        value = 0,
        {
          ...
          for (i in 1:nrow(pmap.df)) {
            setProgress(
              ...
              detail = sprintf(
                "requesting data entered for %s in %s (%d of %d requests)",
                # length(pmap.df[i, 3]) ,
