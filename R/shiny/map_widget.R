...
ous.file = reactive({
  ...
  if (file.exists(paste0(data.folder(), 'newOUS.rds'))) {
    ...
    ous.files = dir.files[grepl('geoFeatures', dir.files, fixed = TRUE)]
...
ous = reactive({
  print('reading ous file')
  req(ous.file())
  if (!any(file.exists(ous.file()))) {
    return(NULL)
  }
  # showNotification("reading geo features file")
  tic()
  o = readRDS(ous.file())
  print('finished reading ous file')
  toc()
...
checkboxInput(
  "show_map",
  label = 'Display map (may take a minute...)',
  value = FALSE
)
...
hf = reactive({
  ous() %>% filter(feature %in% "POINT")
})
admins = reactive({
  ous() %>% filter(feature %in% c('MULTIPOLYGON', 'POLYGON'))
})
fs_selected = reactive({
  ous() %>% filter(orgUnit %in% selectedOUs())
})
...
output$Map.info <- renderText({
  ...
  paste0(
    "click: ",
    xy_str(input$map1_click),
    "dblclick: ",
    xy_str(input$map1_dblclick),
    "hover: ",
    xy_str(input$map1_hover),
    "brush: ",
    xy_range_str(input$map1_brush)
  )
})
