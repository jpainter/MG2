...
monthly.outlier.summary = function(
  df.ts,
  ...
  errors_by_data_and_algorithm =
    ...
    group_by(Month, data, Alg) %>%
    summarise(e = sum(original), n.e = n(), .groups = 'keep')
  ...
  data.totals_by_data =
    ...
    filter(effectiveLeaf) %>%
    group_by(Month, data) %>%
    summarise(t = sum(original), n.t = n(), .groups = 'keep')
...
outlier.summary.chart = function(grand.data.error.totals) {
  g = grand.data.error.totals %>%
    ggplot() +
    geom_line(aes(x = as.factor(month), y = pe, color = Alg, group = Alg)) +
    # scale_y_log10() +
    facet_grid(year ~ data, labeller = label_wrap_gen()) +
    geom_text(
      data = yearly.outlier.summary(grand.data.error.totals),
      aes(
        x = 8,
        y = .65,
        label = paste(
          '%Err(n) =',
          percent(pn, accuracy = 0.1),
          '\n',
          '%Err(value) =',
          percent(pe, accuracy = 1)
        )
      ),
      size = 3
    ) +
