...
ingest_formula_data = function(
  ...
  if (pivotWider | summary) {
    data = data %>%
      pivot_wider(names_from = de, values_from = SUM)
  ...
  if (summary) {
    ...
    if (any(grepl('sum', names(sd), ignore.case = T))) {
      sd = sd %>% rename(Total = sum)
    }
    ...
    data = left_join(data, summaryData, by = c('orgUnit', 'period'))
  }
