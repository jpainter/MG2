...
dqa_reporting_plot = function(data, text_size = 18) {
  ...
  g = ggplot(
    data = data,
    aes(x = as.character(Year), y = pr, label = label, group = 1)
  ) +
...
dqa_mase_plot = function(data) {
  ...
  g = ggplot(
    data = data,
    aes(x = as.character(Year), y = Mean_MASE, label = label, group = 1)
  ) +
    geom_line(linewidth = 1.25) +
    geom_text(vjust = -1, size = 6) +
    scale_y_continuous(
      labels = scales::percent,
      limits = c(0, 1.5 * max(data$Mean_MASE, na.rm = T))
    ) +
    labs(
      x = "Year",
      y = "Percent",
