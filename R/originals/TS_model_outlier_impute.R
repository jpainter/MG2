...
sdhm <- function(x) {
  sqrt((mean(1 / x, na.rm = TRUE))^(-4) * var(1 / x, na.rm = TRUE) / length(x))
}
...
ts_model = function(
  ts, # ts = datasets.  May or may not have nested dataset, ts
  pb = NULL,
  missing.value = NA,
  min.months = 24,
  augment = FALSE,
  ...
  non_missing_groups = ts %>%
    as_tibble() %>%
    group_by(var, orgUnit) %>%
    summarise(m = all(is.na(value)), n = n(), .groups = 'drop') %>%
    filter(m == FALSE, n >= min.months)

  ts = semi_join(ts, non_missing_groups, by = c('var', 'orgUnit'))

  if (nrow(ts) < min.months) {
    return()
  }
  ...
  options(warn = -1) # turn off warnings when model will not fit
  ...
  if (!is.null(covariates)) {
    arima.formula = as.formula(
      paste(y, "~", paste(paste(covariates, collapse = "+"), pdq, sep = "+"))
    )
  } else {
  ...
  if (augment) {
    return(list(
      ...
      ts %>%
        select(-value) %>%
        left_join(decompose %>% augment, by = c('orgUnit', 'var', 'Month'))
    ))
  }
