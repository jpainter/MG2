...
ous_tree = function(ous, ouLevels) {
  ...
  if ('data.frame' %in% class(ous$parent)) {
    ...
    ous.id_parent = ous %>%
      filter(
        !is.na(parent), # Removes top level (no parent)
      ) %>%
      arrange(level) %>%
  ...
  dft = ToDataFrameTree(
    ...
    level = function(x) as.integer(x$level)
  )[, -1] %>%
    as_tibble()
  ...
  dft = dft %>%
    select(orgUnit, all_of(keep.col.numbers), level) %>%
...
data_1 = function(
  ...
  if (!"periodType" %in% names(data)) {
    formula_elements$periodType = "Monthly"
  }
  ptype = min(formula_elements$periodType, na.rm = T)
  if (is_null_or_empty(ptype)) {
    ptype = "Monthly"
  } # if missing, assume Monthly
  cat('\n - ptype is', ptype)
