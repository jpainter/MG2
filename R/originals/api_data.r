...
get = function(
  ...
  if (exists('baseurl')) {
    if (is.character(username) && !loginDHIS2(baseurl, username, password)) {
      return('no connection')
    }
  } else {
    return('missing or incorrect instance url')
  }
...
date_code = function(
  ...
  if (currentMonth) {
    ...
    previous_month <- seq(as.Date(Sys.yearmon()), length = 2, by = "-1 month")[
      2
    ] %>%
      format("%b %Y")
  }
...
api_formula_elements = function(formulaName, dir = NULL, ...) {
  ...
  de = filter(formula.elements, Formula.Name %in% formulaName) %>%
...
most_recent_file = function(file_list_with_date) {
  ...
  download_date = map(rdsFileSplit, ~ str_split(.x[length(.x)], "\\.")[[1]]) %>%
    map_chr(1)

  dates = map_chr(download_date, ~ anydate(.x))

  if (identical(dates, character(0))) {
    return(NA)
  }
...
metadata.file = function(dir = NULL, country = country) {
  ...
  paste0(dir, files('meta', dir = dir, country = country)) %>%
    most_recent_file
}
...
OrgUnitLevels = function(metadata.file = NULL) {
  ...
  read_excel(metadata.file, sheet = 'OrgUnitLevels')
}
OrgUnits = function(metadata.file = NULL) {
  if (is.null(metadata.file)) {
    metadata.file = metadata.file()
  }
  read_excel(metadata.file, sheet = 'OrgUnits')
}
DataElements = function(metadata.file = NULL) {
  if (is.null(metadata.file)) {
    metadata.file = metadata.file()
  }
  read_excel(metadata.file, sheet = 'DataElements')
}
...
fetch_get <- function(
  ...
  fetch = retry(get(
    ...
    password = password,
    .print = get.print
  )[[1]]) # if time-out or other error, will retry
  t = toc(quiet = TRUE)
  ...
  if (is.data.frame(fetch)) {
    ...
    unneeded.cols = which(
      cols %in% c('storedBy', 'created', 'lastUpdated', 'comment')
    )
    ...
    if (get.print) {
      cat("\n", de., periods., orgUnits., aggregationType., ": no records")
    }
  }
...
translate_fetch = function(df, formulaElements = NULL, ous = NULL) {
  ...
  multiple_dataElements = count(formulaElements, dataElement) %>%
    ...
    nrow ==
    0
  multiple_categoryOptionCombos = count(
    formulaElements,
  ...
  if (
    (!multiple_dataElements | !multiple_categoryOptionCombos) &
      !all(is.na(df$categoryOptionCombo))
  ) {
    ...
    de.coc = formulaElements %>%
      ...
      mutate(dataElement.id = dataElement.id %>% str_trim, Categories = "")
  }

  .by = c("dataElement.id", "categoryOptionCombo.ids")
  if (all(is.na(df$categoryOptionCombo))) {
    .by = "dataElement.id"
  }
  ...
  translated_df = df %>%
    ...
    left_join(
      de.coc %>% select(dataElement.id, dataElement) %>% distinct,
      by = 'dataElement.id'
    ) %>%
    left_join(
      de.coc %>% select(categoryOptionCombo.ids, Categories) %>% distinct,
...
api_data = function(
  ...
  if (periodType == 'Weekly') {
    periods = date_code_weekly(YrsPrevious = YrsPrevious)
  }
  ...
  if (
    update && (file_test("-f", previous_dataset_file) | !is_empty(prev.data))
  ) {
    ...
    if (periodType == 'Weekly') {
      periods = date_code_weekly(YrsPrevious = check_previous_years)
    }
  ...
  if (print) {
    cat('\n Making', nrow(pmap.df), "data requests", "\n")
    cat('\n glimpse( pmap.df ) ')
    print(head(pmap.df, n = 10))
  }
  ...
  if (shinyApp == TRUE) {
    if (parallel) {
      ...
      cat(
        '\n',
        'of these, there was no value for',
        sum(is.na(d$SUM)),
        'records \n'
      )
        ...
        value = 0,
        {
          ...
          for (i in 1:nrow(pmap.df)) {
            setProgress(
              ...
              detail = sprintf(
                "requesting SUM and COUNT for %s in %s in %s (%d of %d requests)",
                # length(pmap.df[i, 3]) ,
            ...
            if (
              i > 1 &&
                pmap.df$Var2[i - 1] %in% "LEVEL-1" &&
                (is.na(d[[i - 1]]$COUNT) || d[[i - 1]]$COUNT == 0) &&
                !pmap.df$Var2[i] %in% "LEVEL-1"
            ) {
              cat(
                "\n - previous",
                pmap.df[i - 1, 1],
                "was empty so skipping next lower level,",
                pmap.df[i, 1]
              )
              next
            }
