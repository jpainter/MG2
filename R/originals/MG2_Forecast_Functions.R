...
combination_forecasts = function(primary.forecasts, only.models = NULL) {
  ...
  if (!is.null(only.models) && all(only.models$.model %in% model.names)) {
    allf = primary.forecasts %>%
      inner_join(only.models, by = join_by(Intervention, .model))
    return(allf)
  }
  ...
  # list model names
  if (length(combo.model.names) == 0) {
    combo.model.names = only.models$.model
  }
  ...
  for (x in seq_along(combo.model.names)) {
    ...
    split_combination <- function(combination, original_values) {
      ...
      strsplit(combination, split = paste0("(?<=", pattern, ")"), perl = TRUE)[[
        1
      ]]
    }
    ...
    join_vars = setdiff(
      c(index_var(primary.forecasts), key_vars(primary.forecasts)),
      ".model"
    )
    joined_fables <- reduce(
      split_f[indiv.model.names],
      left_join,
      by = join_vars
    )
    ...
    cf <- joined_fables %>%
      ...
      select(
        key_vars(primary.forecasts),
        index_var(primary.forecasts),
        var,
        .mean,
        older,
        samples
  ...
  if (!is.null(only.models)) {
    allf = allf %>% inner_join(only.models, by = join_by(Intervention, .model))
  }
...
best_fables_accuracy = function(
  ...
  best_fables_accuracy = fables_accuracy %>%
    ...
    mutate(
      ...
      percent_diff = 100 *
        (({{ .metric }} - first({{ .metric }})) / first({{ .metric }})) %>%
          round(digits = 3)
    ) %>%
...
differences = function(
  ...
  comparator = data %>%
    filter(Intervention %in% groups[1]) %>%
    pull(WPE)

  vaccinating = data %>%
    filter(Intervention %in% groups[2]) %>%
    pull(WPE)
...
differences_probabilty = function(differences, thresholds = c(0, -10, -20)) {
  ...
  tibble(Threshold = thresholds, `Probability less than` = probability)
}


## Facility characteristics functions ####

facility_characteristics = function(
  dataset,
  ...
  if (any(.selected == FALSE)) {
    ...
    data = dataset %>% filter(year(Month) < 2023) %>% mutate(selected = TRUE)
  }
    ...
    errors = map2_df(
      ...
      ~ bind_cols(
        ...
        outlier.summary.tibble(
          data = data %>%
            filter(Intervention == .x, selected == .y),
          .print = FALSE
        ) %>%
  ...
  d = mvip.facilities %>%
    st_drop_geometry() %>%
    mutate(
      selected = orgUnit %in% unique(dataset$orgUnit),
    ...
    summarise(
      ...
      Under5.p = paste0(
        "(",
        percent(Under5 / sum(.$under5, na.rm = TRUE)),
        ")"
      ),
      vx.doses = sum(total.doses, na.rm = TRUE),
      vx.doses.p = paste0(
        "(",
        percent(vx.doses / sum(.$total.doses, na.rm = TRUE)),
        ")"
      ),
  ...
  if (errs) {
    d = d %>%
      left_join(
        errors %>% select(Intervention, selected, `%N`, `%Total Value`),
...
facilities.reporting = function(.dataset) {
  reporting =
    bind_rows(
      .dataset %>%
        filter(Intervention %in% 'Vaccinating') %>%
        dqaNumberReporting() %>%
...
dataset.consisency = function(.dataset) {
  ...
  consistency %>%
    ...
    scale_y_continuous(
      limits = c(0, max(consistency$n_frequently_reporting) + 10)
    ) +
    theme_minimal(base_size = 12) +
    facet_grid(Intervention ~ .)
}
...
dataset = function(
  ...
  if (!"tbl_ts" %in% class(data)) {
    ...
    data = tsibble(data, index = 'Month', key = groups)
  }
...
tsmodels = function(
  ...
  if (
    !is.na(type) &&
      !type %in% c('transform and covariate', 'transform', 'covariate')
  ) {
    cat(
  ...
  if (type %in% 'transform') {
    pre.intervention.train.primary.models <- train_data %>%
      model(
        ...
        p1 = prophet(log(var + 1) ~ season("year", 1, type = "multiplicative")),
        p4 = prophet(log(var + 1) ~ season("year", 4, type = "multiplicative")),
        p8 = prophet(log(var + 1) ~ season("year", 8, type = "multiplicative"))
      )
  }
  ...
  if (msg) {
    cat(
      '\n - tsmodels: Primary models finished.',
      t$callback_msg,
  ...
  pre.intervention.train.primary.forecasts =
    ...
    forecast(
      h = numberForecastMonths, # new_data = test_data ,
      times = n_forecasts
    ) %>%
...
plotActualPredicted = function(actual, predicted, .var = "younger", ...) {
  ...
  group_by_vars = c(key_vars(predicted), index_var(predicted))
...
diffHistogram = function(actual, predicted, xlimits = c(NA, NA), ...) {
  ...
  d = diffPredictedActual %>%
    inner_join(diffPredictedActual.summary, by = join_by(.model)) %>%
...
impactHistogram = function(actual, predicted, ...) {
  ...
  impact.reps(actual, predicted, ...) %>%
    ...
    geom_histogram(
      data = . %>% filter(WPE >= hdi_lower & WPE <= hdi_upper),
      binwidth = 1,
      fill = "black",
      alpha = 0.5
    ) +
    geom_histogram(
      data = . %>% filter(round(WPE) == round(median)),
      binwidth = 1,
      fill = "blue",
      alpha = 0.5
    ) +
    scale_x_continuous(limits = c(-40, 20), breaks = seq(-40, 20, 5)) +
    labs(
...
impactTable = function(
  ...
  impactTable =
    bind_rows(
      diff.summary(actual, predicted, grouping = grouping, .var = .var, ...),
