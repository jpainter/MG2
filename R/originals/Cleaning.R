  ...
  if (is.null(key_entry_errors)) {
    ...
    large_values =
      count(
        as_tibble(
          d %>%
            filter(nchar(original) > 3, effectiveLeaf)
        ) %>%
          ungroup,
        original
      ) %>%
    ...
    key_entry_errors = large_values %>%
      filter(
        n >
          3 *
            median(
              large_values %>% filter(row_number() < 11) %>% pull(n)
            )
      ) %>%
      pull(original)
...
outlier.summary.tibble = function(
  ...
  if ('expected' %in% cols) {
    cols = setdiff(cols, 'expected')
  }

  if (!'mad5' %in% names(data)) {
    cols = setdiff(cols, 'mad5')
  }

  if (!'AllSmall' %in% names(data)) {
    cols = setdiff(cols, 'AllSmall')
  }
