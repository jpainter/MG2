...
most_recent_file = function(file_list_with_date) {
  ...
  download_date = map(
    rdsFileSplit,
    ~ .x[anydate(.x) %>% is_date() %>% which %>% max]
  ) %>%
    unlist

  if (identical(download_date, character(0))) {
    return(NA)
  }

  # most recent file
  file = file_list_with_date[which(
    download_date == max(download_date, na.rm = T)
  )]
...
extremely_mad = function(
  ...
  if (logical) {
    return(!extreme & !(over_max | key_error))
  } # FALSE = outlier
  y[extreme] = NA
  return(y)
}
