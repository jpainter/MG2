...
correct_incidence = function(
  ...
  B = (F - D * (1 - beta) / beta) /
    ((1 - beta) / beta + TPR_nontested / (1 - TPR_nontested) + 1)
  ...
  corrected_incidence = (C + E) /
    pop *
    (standardincidence / nonmalariafever_incidence) *
    1000 # without attributable correction
  corrected_incidence2 = (C + E) *
    lambda /
    pop *
    (standardincidence / nonmalariafever_incidence) *
    1000 # with attributable correction
...
adjCorrInc = function(
  ...
  if (
    is.na(confirmedMalaria) | is.na(numberTested) | is.na(allCauseAttendance)
  ) {
    return(NA)
  }
  if (
    (confirmedMalaria >= numberTested) | (numberTested >= allCauseAttendance)
  ) {
    return(NA)
  }

  reportedNonMalariaFeverRate = (numberTested - confirmedMalaria) /
    (allCauseAttendance - confirmedMalaria)
  ...
  if (reportedNonMalariaFeverRate >= expectedNonMalariaFeverRate) {
    return(reportedMalariaRate)
  }

  #Otherwise, add expected cases from untested patients
  adjMalariaRate = (confirmedMalaria + unTestedNonMalariaFever * tpr) /
    allCauseAttendance
...
adjCorrCases = function(
  ...
  # return null if values don't make sense
  if (
    any(is.na(confirmedMalaria), is.na(numberTested), is.na(allCauseAttendance))
  ) {
    return(NA)
  }
  if (
    any(confirmedMalaria >= numberTested, numberTested >= allCauseAttendance)
  ) {
    return(NA)
  }
