...
df.ts = function(df, period = "Month", missing.value = NA) {
  ...
  ts = df %>%
    mutate(COUNT = as.numeric(COUNT), SUM = as.numeric(SUM)) %>%
...
df_pre_ts = function(df, period = "Month", missing.value = NA) {
  ...
  if (period %in% c("Week", "Weekly")) {
    .period = "Week"
  }
  if (period %in% c("Month", "Monthly")) {
    .period = "Month"
  }
...
df_ts = function(
  ...
  cat('\n - as_tsibble')
  ts = df.pre.ts %>%
    as_tsibble(key = c(orgUnit, data.id), index = !!.period, validate = FALSE)
...
mable_data = function(
  ...
  #Testing
  if (testing) {
    saveRDS(data.agg.ts, 'data.agg.ts.rds')
  }
  ...
  if (!is.null(covariates)) {
    fit = train.data. %>%
      model(
        ...
        nnetar.tr.cov = NNETAR(
          fabletools::box_cox(total, lambda = !!{{ .lam }}),
          lambda = .lam,
...
forecast_function = function(fit, key, test.data, .times = 100, .cat = FALSE) {
  fit. = fit %>% semi_join(key, by = names(key))
