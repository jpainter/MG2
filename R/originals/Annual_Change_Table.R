...
yearly_summary_table <- function(
  ...
  ft <- yearly_data %>%
    ...
    color(
      i = which(
        yearly_data$`Change Direction` %in%
          c("positive", "negative", "baseline")
      ),
      j = "Formatted Change",
      color = "white"
    ) %>%
  ...
  # cat("\n=== SUMMARY STATISTICS ===\n")
  valid_changes <- yearly_data$`Percent Change`[
    !is.na(yearly_data$`Percent Change`)
  ]
