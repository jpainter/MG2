...
checkboxInput("scale", label = 'Scale values (x-mean)/sd + 1)', value = FALSE)

checkboxInput('components', label = 'Visualize trend', value = FALSE)

checkboxInput("forecast_ci", label = 'Confidence interval', value = FALSE)

### Administrative Hierarchy

selectInput("agg_level", label = "Aggregate to", choices = NULL, selected = 1)


checkboxInput("facet_admin", label = "Facet by admin", value = FALSE)

checkboxInput("facet_split", label = "Facet by split", value = FALSE)

checkboxInput("selected", label = 'Selected facilities only', value = TRUE)


checkboxInput("legend", label = 'Show legend', value = FALSE)

checkboxInput("plotly", label = 'Plotly Chart', value = FALSE)
...
textInput('model.formula', 'Model Formula', value = 'total ~ 1')

textInput('covariates', 'Model covariates', value = NULL)
...
observe({
  updateSelectInput(session, 'level5', choices = level5(), selected = NULL)
})
...
observe({
  updateSelectInput(session, 'var_y', choices = names(trendData()))
})
...
tsModel = reactive({
  ...
  print('tsModel():')
  print(paste('available vars:', paste(names(trendData()), collapse = ',')))
...
plotTrends = reactive({
  ...
  num_agg_levels = count(.d %>% as_tibble, !!rlang::sym(input$agg_level)) %>%
    nrow()
  ...
  if (num_agg_levels > 1 & input$facet_admin) {
    print('facets')
    g = g +
      facet_wrap(vars(!!rlang::sym(input$agg_level)), scales = "free_y")
  }
  ...
  if (input$pre_evaluation) {
    ...
    g = g +
      ...
      geom_vline(
        xintercept = as.Date(pre_eval_date),
        color = 'brown',
        alpha = .25
      ) +
      # annotate( "text" ,
...
mpe_distribution = function(forecast_distribution, var) {
  ...
  forecast_distribution %>%
    unnest(.fore) %>%
    group_by({{ index }}) %>%
    mutate(n = row_number(), .fore = ifelse(is.na({{ var }}), NA, .fore)) %>%
    group_by(n) %>%
    summarise(
    ...
    ungroup() %>%
    mutate(mpe = (actual - .fore) / .fore, mape = abs(actual - .fore) / .fore)
}
...
mpe_plot = function(.data, var, rope_range = NULL) {
  ...
  cri_label = paste(
    ...
    " - ",
    percent(cri[2], accuracy = .1),
    "]",
    sep = ""
  )

  mpe_rope = rope(me, range = rope_range)
  mpe_rope_label = paste(
    'Probabality in ROPE*\n [',
  ...
  g =
    ...
    labs(
      title = 'Posterior Distribution of Error',
      # subtitle = {{ var }} ,
      y = 'Probability'
      # x = {{ var }} ,
...
sigData = reactive({
  ...
  print('forecast_distribution() r.names:')
  print(r.names)
...
output$plot_significance <- renderPlot({
  plotSig()
})
