...
Raw DHIS2 data cannot be used directly for program decisions. Facilities submit incomplete reports, commodity stockouts suppress case counts, data entry errors create biological implausibilities, and administrative restructuring disrupts time series — all of which distort aggregate indicators without any visible signal in standard DHIS2 outputs. The [WHO Data Quality App](https://dhis2.org/who-data-quality-tool/) instructs analysts to adjust for incomplete reporting before trend analysis, but provides no tool to do so.
