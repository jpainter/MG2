...
| Object | Description |
|---|---|
| `mg2_demo` | Raw API-style data — one row per facility × element × month × category. Format matches `api_data()` output. |
...
**Time range:** 60 months — 12 real months (Jan–Dec 2025) downloaded from the public [DHIS2 Sierra Leone demo instance](https://play.im.dhis2.org/stable-2-41-8/) plus 48 months of prior history generated by seasonal bootstrap (tile the 12 real months four times with lognormal multiplicative noise, SD ≈ 12%).
